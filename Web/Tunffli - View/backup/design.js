...
function txt(){
  var t = "Tunffli".split("").join(String.fromCharCode(0x2004));
  $.font = "3.5em Philosopher";
  $.fillStyle = 'hsla(35,84%,52%,1)';
  $.fillText(t, (c.width - $.measureText(t).width) * 0.5, c.height * 0.5);
}

function draw() {
 $.fillStyle = 'hsla(0,0%,7%,.45)';
...
function conn(p1, p2) {
  $.strokeStyle = 'hsla(35,84%,52%,1)';//hsl(35, 84%, 52%)
...
jQuery(".scroll").click(function(event){
				...
				jQuery('html,body').animate({scrollTop:jQuery("#subheader").offset().top},1200);
			});
});
